...
// ─── Parse directive options string ──────────────────────────────────────────
// e.g. "path/to/file.md | width: 500 | caption: My figure"
// → { _path: "path/to/file.md", width: "500", caption: "My figure" }
...
function extractModifiers(line) {
  ...
  // Support trailing grouped modifiers without consuming non-modifier inline tags.
  // Examples supported:
  //   "Title {.no-num .center}"
  //   "Title {.no-num} {.center}"
  // and safe with inline style syntax like "{size:22}X{/size} {.center}".
  while (true) {
    ...
    for (const mm of modMatches) {
      modifiers.add(mm[1].toLowerCase());
    }
...
function parseMD(text, dir, R, importFn, opts = {}) {
  ...
  // If vars changed (document defines vars), update renderer's var store
  // (applyVars inside R already uses the passed-in vars from config — document vars
  // must be passed as opts.vars so they're pre-merged before rendering)
  ...
  function addAnnexTitle(title, alignment) {
    push(R.makeElementTitle ? R.makeElementTitle("annex", title, { alignment, state }) : consumeCaption("annex", title, alignment));
  }
  ...
  while (i < lines.length) {
    ...
    // ── Flush pending annex title when non-empty non-directive line appears ──
    ...
    if (directiveMatch) {
      ...
      if (verb === "element") {
        ...
        // Support both old positional "figure | title: …" and new "type: figure | title: …"
      ...
      if (verb === "section") {
        ...
        // Emit a section break marker object (consumed by builder to split docx sections)
        elements.push({
          _type:       "SECTION_BREAK",
      ...
      // @var: name = value  (already collected in first pass; skip at render time)
      if (verb === "var") {
        i++; continue;
      }
    ...
    if (imgMatch) {
      ...
      if (fs.existsSync(absImg)) {
        ...
        if (pendingStyle && R.makeCallout) {
          // Callout wrapping images is unusual — skip wrapping
          pendingStyle = null;
        }
        push(imgElems);
    ...
    if (trimmed.startsWith("> ") || trimmed === ">") {
      ...
      if (pendingStyle) {
        ...
        push(R.makeCallout(content, style, alignment ? { alignment } : {}));
      } else {
    ...
    if (hMatch) {
      ...
      // Keep heading alignment independent from default paragraph alignment.
      // This preserves centered `.no-num` headings (handled by renderer) unless
      // alignment is explicitly requested via modifiers or align stack.
    ...
    // Deprecated {.box} inline — remap to pendingStyle
